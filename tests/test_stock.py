...
def setup_db(session):
    session.execute(
        """CREATE TABLE Product
                             (_id uuid, nome string,type string, qtde int, cost float,status string)"""
    )  # 3
    session.execute(
        'INSERT INTO Product VALUES ("066de609-b04a-4b30-b46c-32537c7f1fja","Macbook Pro M2","Notebook",22,10.000,"Disponivel")'
    )  # 4
