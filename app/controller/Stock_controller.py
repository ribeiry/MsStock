...
@router.get(
    "/",
    response_description="List all products of the Stock",
...
@router.get(
    "/{id}",
    response_description="Get an Product in the Stock",
    status_code=status.HTTP_200_OK,
...
@router.put(
    "/{id}/sub",
    response_description="Take a Product a Stock",
    status_code=status.HTTP_200_OK,
    response_model=Product,
)
async def update_stock(id: str, request: Request):
    ...
    if new_qtde == -1:
        raise HTTPException(
            ...
            detail=f"Product with ID: {id} don`t have qtde",
        )
...
@router.put(
    "/{id}/add",
    response_description="Return a Product a Stock",
    status_code=status.HTTP_200_OK,
    response_model=Product,
)
